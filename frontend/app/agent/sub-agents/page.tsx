...
export default function AgentSubAgents() {
    ...
    const [activeTab, setActiveTab] = useState<"overview" | "invite" | "training">("overview");
    ...
    return (
        <div className="animate-fade-in-up space-y-8 relative z-10 text-left">
            ...
            <section className="p-8 rounded-[2.5rem] bg-white border border-[#6605c7]/10 shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                ...
                <div className="flex gap-3 flex-wrap">
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-3 bg-[#6605c7] text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#6605c7]/90 transition-all shadow-lg shadow-[#6605c7]/20"
                    >
                        <span className="material-symbols-outlined text-sm">person_add</span>
                        + Invite New Sub-Agent
                    </button>
                    <button
                        onClick={() => setActiveTab("training")}
                        className="flex items-center gap-2 px-5 py-3 bg-white border border-[#6605c7]/20 text-[#6605c7] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#6605c7]/5 transition-all"
                    >
                        <span className="material-symbols-outlined text-sm">bar_chart</span>
                        View All Sub-Agent Activity
                    </button>
                </div>
            </section>
            ...
            <div className="flex gap-2 border-b border-gray-100 pb-1">
                {[
                    { id: "overview", label: "9.1 Overview Table", icon: "table" },
                    { id: "invite", label: "9.2 Invite & Onboard", icon: "person_add" },
                    { id: "training", label: "9.3 Training & Access", icon: "school" },
                ].map((tab) => (
                    <button
                        key={tab.id}
            ...
            {activeTab === "overview" && (
                <section className="bg-white border border-[#6605c7]/10 p-8 rounded-[2.5rem] shadow-sm space-y-6">
                        ...
                        <table className="w-full text-left text-xs border-collapse">
                            <thead>
                                <tr className="bg-gray-50 text-gray-400 border-b border-gray-100 font-black uppercase tracking-wider text-[9px]">
                                    ...
                                    <th className="p-4 text-center">Leads (MTH)</th>
                            ...
                            <tbody className="divide-y divide-gray-50 font-bold text-gray-700">
                                {displaySubAgents.map((sa: any, i: number) => (
                                    <tr key={i} className="hover:bg-[#6605c7]/3 transition-colors group">
                                        <td className="p-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-black text-gray-900 text-xs">{sa.name}</p>
                                                    <p className="text-[9px] text-gray-400 font-bold uppercase">{sa.email || "—"}</p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className="p-4">
                                            <div className="flex items-center justify-center gap-2">
                                                <button
                                                    ...
                                                    className="px-3 py-1.5 bg-[#6605c7]/5 text-[#6605c7] rounded-lg text-[9px] font-black uppercase hover:bg-[#6605c7] hover:text-white transition-all"
                                                >
                                                    View
                                                </button>
                                            </div>
                    ...
                    <div className="p-4 rounded-2xl bg-indigo-50/70 border border-indigo-100 flex items-center justify-between">
                        <span className="text-[10px] font-black text-[#6605c7] uppercase tracking-widest">Override Commission from Sub-Agents</span>
                        <span className="text-sm font-black text-[#6605c7]">₹{displaySubAgents.reduce((acc: number, sa: any) => acc + sa.myCut, 0).toLocaleString()} — included in June Net Payable</span>
                    </div>
                </section>
            )}

            {/* 9.2 Invite Sub-Agent */}
            {activeTab === "invite" && (
                <section className="bg-white border border-[#6605c7]/10 p-8 rounded-[2.5rem] shadow-sm max-w-2xl space-y-6">
                    <div>
                    ...
                    <form onSubmit={handleInviteSubAgent} className="space-y-5">
                        ...
                        <div className="p-4 bg-amber-50/60 border border-amber-100 rounded-xl text-[10px] font-black text-amber-700 uppercase tracking-wider text-center">
