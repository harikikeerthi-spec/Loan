...
export class AiSearchController {
  ...
  async search(@Body() body: any) {
    ...
    if (!API_KEY) {
      return type === 'university_detail' ? { success: true, university: null } : { success: true, universities: [] };
    }
    ...
    try {
      ...
      if (groqResp.ok) {
        ...
        return { success: true, universities: parsed.universities || parsed.results || [] };
      } else {
        const errorText = await groqResp.text();
        console.error('Groq API Error on Backend:', errorText);
        return { success: false, error: 'Groq API returned an error', universities: [] };
      }
    } catch (e: any) {
      console.error('Groq call failed on Backend', e);
