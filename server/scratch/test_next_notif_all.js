...
async function testAll() {
    ...
    console.log(`Found ${users.length} users. Testing Next.js notifications API (port 3000) for each...`);
    ...
    for (const user of users) {
        ...
        if (res.status === 500) {
            console.log(`User: ${user.email} (Role: ${user.role}) -> Status: ${res.status}, Body: ${res.body}`);
        }
    }

    console.log('Done testing Next.js notifications API.');
    await app.close();
}
